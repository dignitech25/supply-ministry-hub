-- Fix ambiguous column reference in create_quote_with_items function
...
begin
  ...
  -- Compute subtotal with explicit table alias to avoid ambiguity
  select coalesce(sum(qi.quantity * coalesce(qi.unit_price, 0)), 0)
    into v_subtotal
